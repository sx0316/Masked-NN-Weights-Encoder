...
def append_model_name(string, model_name):
    return model_name + "_" + string


# 1. Generate a series of masked matrices
def load_pretrained_weight_matrices(mask_ratio, model):
    ...
    for name, param in pretrained_model.named_parameters():
        ...
        if 'bn' in name:
            ...
            # res = res.unsqueeze(0)
            layer_name = append_model_name(name.split(".bn")[0], model)
            ...
            layer_name = append_model_name(layer_name, model)
...
class RNNDataloader(dataloader.DataLoader):
    ...
    def __init__(self, data, batch_size, shuffle=True, num_workers=1, pin_memory=False, biases={}):
        ...
        self.resize_sz = 32
        src_dataset = CIFAR10(root='./data', train=True, download=True, transform=transforms.Compose(
...
def train_weight_predictor():
    ...
    data1, biases1 = load_pretrained_weight_matrices(mask_ratio, "resnet34")
    data2, biases2 = load_pretrained_weight_matrices(mask_ratio, "resnet18")
    data = data1 + data2
    ...
    validation_dataloader_ = RNNDataloader(val_set, batch_size=batch_size, pin_memory=pin_memory, biases=biases,
                                           num_workers=5)
    tmodel = train(model, dataloader_, validation_dataloader_, num_epochs=epochs, learning_rate=lr, logging=logging,
